...
import {
  ...
  CheckCircle2, Clock, Ban, UserCheck, Eye, Plus, Minus,
} from "lucide-react";
...
interface UserProfile {
  ...
  email: string;
  full_name: string;
  wallet_balance: number;   // pulled from wallets.balance
  is_banned: boolean;
...
interface Transaction {
  ...
  plan_name: string;
  amount: number;
  phone_number: string;  // FIX: your column is phone_number not phone
...
const AVATAR_COLORS = ["rgba(83,230,212,0.2)", "rgba(250,204,21,0.2)", "rgba(74,222,128,0.2)", "rgba(167,139,250,0.2)", "rgba(251,146,60,0.2)"];
const AVATAR_TEXT   = ["#53E6D4", "#FACC15", "#4ade80", "#a78bfa", "#fb923c"];
function avatarColor(id: string)     { let h = 0; for (const c of id) h = (h * 31 + c.charCodeAt(0)) % AVATAR_COLORS.length; return AVATAR_COLORS[h]; }
function avatarTextColor(id: string) { let h = 0; for (const c of id) h = (h * 31 + c.charCodeAt(0)) % AVATAR_TEXT.length;   return AVATAR_TEXT[h]; }

// ─── Top-up Modal ─────────────────────────────────────────────────────────────

function TopUpModal({ user, onClose, onDone }: { user: UserProfile; onClose: () => void; onDone: (newBal: number) => void }) {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    const newBalance = type === "credit" ? user.wallet_balance + val : user.wallet_balance - val;
    try {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#0D1F1F", border: "1px solid rgba(83,230,212,0.15)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 420 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", color: "rgba(244,247,247,0.4)", cursor: "pointer" }}><X style={{ width: 18, height: 18 }} /></button>
        </div>
        ...
        <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Custom amount (₦)"
          style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "1px solid rgba(83,230,212,0.2)", background: "rgba(83,230,212,0.04)", color: "#F4F7F7", fontSize: 14, fontFamily: "'IBM Plex Mono', monospace", marginBottom: 10, boxSizing: "border-box", outline: "none" }} />
        ...
        <button onClick={handleSubmit} disabled={saving}
          ...
          {saving && <Loader2 style={{ width: 15, height: 15, animation: "spin .8s linear infinite" }} />}
...
function BanModal({ user, onClose, onDone }: { user: UserProfile; onClose: () => void; onDone: (banned: boolean) => void }) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#0D1F1F", border: `1px solid ${isBanned ? "rgba(74,222,128,0.2)" : "rgba(248,113,113,0.2)"}`, borderRadius: 20, padding: 28, width: "100%", maxWidth: 400 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <h2 style={{ fontSize: 16, fontWeight: 800, color: "#F4F7F7" }}>{isBanned ? "Unban User" : "Ban User"}</h2>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "rgba(244,247,247,0.4)", cursor: "pointer" }}><X style={{ width: 18, height: 18 }} /></button>
        </div>
        <p style={{ fontSize: 13, color: "rgba(244,247,247,0.5)", marginBottom: 18, lineHeight: 1.6 }}>
          {isBanned ? `Restore access for ${user.full_name || user.email}.` : `Prevent ${user.full_name || user.email} from logging in or making transactions.`}
        </p>
        {!isBanned && (
          <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="Ban reason (optional)" rows={3}
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: "11px 0", borderRadius: 12, border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "rgba(244,247,247,0.5)", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Cancel</button>
          <button onClick={handleSubmit} disabled={saving}
            ...
            {saving && <Loader2 style={{ width: 13, height: 13, animation: "spin .8s linear infinite" }} />}
...
function TxDrawer({ user, onClose }: { user: UserProfile; onClose: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", zIndex: 100, display: "flex", justifyContent: "flex-end" }}>
      <div style={{ width: "100%", maxWidth: 500, height: "100%", background: "#0D1F1F", borderLeft: "1px solid rgba(83,230,212,0.12)", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10, padding: "16px 24px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          {[
            { label: "Balance",  value: `₦${fmt(user.wallet_balance)}`, color: user.wallet_balance > 0 ? "#4ade80" : "#f87171" },
            { label: "Spent",    value: `₦${fmt(totalSpent)}`,          color: "#53E6D4" },
            { label: "Total Tx", value: txs.length,                     color: "#FACC15" },
            ...
            <div key={s.label} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: "10px 12px" }}>
              ...
              <p style={{ fontSize: 14, fontWeight: 900, color: s.color, fontFamily: "'IBM Plex Mono', monospace" }}>{s.value}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ flex: 1, padding: "16px 24px" }}>
          ...
          {loading ? (
            ...
            <p style={{ color: "rgba(244,247,247,0.3)", fontSize: 13, textAlign: "center", paddingTop: 40 }}>No transactions yet</p>
          ) : (
                ...
                return (
                  <div key={tx.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 12, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)" }}>
                    <span style={{ padding: "2px 8px", borderRadius: 6, background: nc.bg, border: `1px solid ${nc.border}`, color: nc.color, fontSize: 10, fontWeight: 800, whiteSpace: "nowrap" }}>
                      {normalizeNetwork(tx.network)}
                    </span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 12, fontWeight: 600, color: "#F4F7F7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tx.plan_name || tx.type || "—"}</p>
                      <p style={{ fontSize: 10, color: "rgba(244,247,247,0.3)", marginTop: 2 }}>{tx.phone_number} · {timeAgo(tx.created_at)}</p>
                    </div>
...
export default function AdminUsers() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    // Fetch profiles + wallets + tx summary in parallel
    const [
      { data: profiles },
      { data: wallets },
      { data: txSummary },
      ...
      supabase.from("wallets").select("user_id, balance"),   // FIX: pull from wallets table
      supabase.from("transactions").select("user_id, amount, status"),
    ]);

    if (!profiles) { toast.error("Failed to load users"); setLoading(false); return; }

    // Build wallet map: user_id → balance
    const walletMap: Record<string, number> = {};
    (wallets || []).forEach(w => { walletMap[w.user_id] = w.balance ?? 0; });

    // Build tx map: user_id → { count, spent }
    ...
    (txSummary || []).forEach(t => {
      ...
      if (t.status === "success") txMap[t.user_id].spent += t.amount ?? 0;
    });

    setUsers(profiles.map(p => ({
      ...p,
      wallet_balance: walletMap[p.id] ?? 0,   // FIX: use wallets.balance
  ...
  useEffect(() => {
    ...
    channelRef.current = supabase
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "wallets" },  () => fetchUsers()) // FIX: listen to wallets too
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "wallets" },  () => fetchUsers())
      .subscribe();
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      let av: any = sortKey === "full_name" ? (a.full_name ?? a.email ?? "").toLowerCase() : (a as any)[sortKey] ?? 0;
      let bv: any = sortKey === "full_name" ? (b.full_name ?? b.email ?? "").toLowerCase() : (b as any)[sortKey] ?? 0;
  ...
  function SortIcon({ k }: { k: SortKey }) {
    ...
    return sortAsc ? <ChevronUp style={{ width: 11, height: 11, color: "#53E6D4" }} /> : <ChevronDown style={{ width: 11, height: 11, color: "#53E6D4" }} />;
  }
  ...
  return (
    <>
      <style>{`
        ...
        input::placeholder, textarea::placeholder { color: rgba(244,247,247,0.25); }
      `}</style>

      {topUpUser && <TopUpModal user={topUpUser} onClose={() => setTopUpUser(null)} onDone={(bal) => { setUsers(p => p.map(u => u.id === topUpUser.id ? { ...u, wallet_balance: bal } : u)); setTopUpUser(null); }} />}
      {banUser   && <BanModal   user={banUser}   onClose={() => setBanUser(null)}   onDone={(banned) => { setUsers(p => p.map(u => u.id === banUser.id ? { ...u, is_banned: banned, banned_at: banned ? new Date().toISOString() : null } : u)); setBanUser(null); }} />}
      ...
      <div style={{ color: "#F4F7F7", fontFamily: "'Sora', sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 26, flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "rgba(244,247,247,0.4)" }}>{stats.total} total · {stats.newToday} joined today</p>
          </div>
        ...
        <div style={{ background: "rgba(8,12,12,0.6)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: "16px 18px", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            {FILTERS.map(f => (
              <button key={f.key} className="filt-chip" onClick={() => { setFilter(f.key); setPage(1); }}
                  ...
                  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    {[
                      ...
                      <th key={h.key} onClick={() => toggleSort(h.key)}
                        ...
                        <span style={{ display: "flex", alignItems: "center", gap: 4 }}>{h.label} <SortIcon k={h.key} /></span>
                      </th>
                    ))}
                ...
                <tbody>
                  {paginated.map(user => (
                    <tr key={user.id} className="u-row" style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
                      ...
                      <td style={{ padding: "13px 16px" }}>
                        <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, color: "rgba(244,247,247,0.55)" }}>₦{fmt(user._total_spent ?? 0)}</span>
                      </td>
                      <td style={{ padding: "13px 16px" }}>
                      ...
                      <td style={{ padding: "13px 16px" }}>
                        <span style={{ fontSize: 13, color: "rgba(244,247,247,0.5)", fontFamily: "'IBM Plex Mono', monospace" }}>{user._tx_count ?? 0}</span>
                      </td>
                      <td style={{ padding: "13px 16px" }}>
                        {user.is_banned
                      ...
                      <td style={{ padding: "13px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <button className="act-btn" onClick={() => setTxUser(user)} title="View transactions"
                            ...
                            <Eye style={{ width: 12, height: 12 }} /> Tx
                          </button>
                          <button className="act-btn" onClick={() => setTopUpUser(user)} title="Adjust wallet"
                          ...
                          <button className="act-btn" onClick={() => setBanUser(user)} title={user.is_banned ? "Unban" : "Ban"}
                            ...
                            {user.is_banned ? <ShieldCheck style={{ width: 13, height: 13 }} /> : <ShieldOff style={{ width: 13, height: 13 }} />}
                          </button>
                        </div>
                      </td>
                    </tr>
