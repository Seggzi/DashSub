...
import {
  ...
  Package, CheckCircle2, XCircle, Save,
  Loader2, Percent, Wifi, AlertTriangle,
} from "lucide-react";
...
export default function AdminPlans() {
  ...
  // ── Toggle active via API (bypasses RLS) ──
  async function toggleActive(plan: Plan) {
    ...
    try {
      const res = await fetch("/api/admin/update-plan", {
        ...
        body: JSON.stringify({ id: plan.id, field: "is_active", value: !plan.is_active }),
      });
  ...
  // ── Save price edit via API (bypasses RLS) ──
  async function saveEdit(plan: Plan) {
    ...
    try {
      const res = await fetch("/api/admin/update-plan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: plan.id, field: "selling_price", value: val }),
      });
      const result = await res.json();
  ...
  // ── Apply bulk markup via API ──
  async function applyMarkupToNetwork(percent: number) {
    const count = network === "ALL"
      ? plans.length
      : plans.filter(p => p.network_name === network).length;

    if (!count) return;
    if (!confirm(`Apply ${percent}% markup to ${count} plans${network !== "ALL" ? ` (${network})` : ""}?`)) return;

    setApplyingMarkup(true);
    try {
      const res = await fetch("/api/admin/bulk-markup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ network, percent }),
      });
      const result = await res.json();
      ...
      toast.success(`✅ Applied ${percent}% markup to ${result.updated} plans`);
      await fetchPlans(); // Reload to show new prices
    } catch (err) {
      toast.error("Network error");
    } finally {
  ...
  const profit = (p: Plan) => p.selling_price - p.cost_price;
  const margin = (p: Plan) => p.cost_price > 0
    ? ((profit(p) / p.cost_price) * 100).toFixed(1) : "0";
  ...
  return (
    <>
      <style>{`
        ...
        .price-input { outline: none; }
        .price-input:focus { border-color: #53E6D4 !important; box-shadow: 0 0 0 3px rgba(83,230,212,0.12); }
        .net-chip  { transition: all .14s; cursor: pointer; }
        .net-chip:hover { transform: translateY(-1px); }
        .markup-btn { transition: all .14s; }
        .markup-btn:hover:not(:disabled) { background: rgba(83,230,212,0.15) !important; transform: translateY(-1px); }
        .toggle-btn { transition: all .15s; cursor: pointer; }
        .toggle-btn:hover:not(:disabled) { filter: brightness(1.2); }
        .edit-price-btn { transition: all .13s; }
        .edit-price-btn:hover { background: rgba(83,230,212,0.08) !important; }
            ...
            {NETWORKS.map(n => {
              ...
              return (
                <button key={n} className="net-chip"
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 20, border: `1.5px solid ${active ? (nc?.border ?? "rgba(83,230,212,0.3)") : "rgba(255,255,255,0.08)"}`, background: active ? (nc?.bg ?? "rgba(83,230,212,0.12)") : "transparent", color: active ? (nc?.color ?? "#53E6D4") : "rgba(244,247,247,0.45)", fontSize: 12, fontWeight: 800 }}>
                  {n}
                      ...
                      <tr key={plan.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                          ...
                          {isEdit ? (
                            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                              ...
                              <button
                                ...
                                disabled={isSaving}
                                style={{ padding: "6px 12px", borderRadius: 8, background: "rgba(74,222,128,0.15)", border: "1px solid rgba(74,222,128,0.25)", cursor: isSaving ? "not-allowed" : "pointer", color: "#4ade80", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 4 }}>
                                {isSaving ? <Loader2 style={{ width: 12, height: 12 }} className="spin" /> : <Save style={{ width: 12, height: 12 }} />}
                                {isSaving ? "Saving…" : "Save"}
                              </button>
                              <button
                                onClick={() => setEditingId(null)}
                                style={{ padding: "6px 10px", borderRadius: 8, background: "rgba(248,113,113,0.12)", border: "1px solid rgba(248,113,113,0.2)", cursor: "pointer", color: "#f87171", fontSize: 12, fontWeight: 700 }}>
                                ✕
                              </button>
                            </div>
                          ) : (
                        ...
                        <td style={{ padding: "13px 16px" }}>
                          <button
                            ...
                            disabled={isToggling}
                            style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", borderRadius: 8, border: "none", cursor: isToggling ? "not-allowed" : "pointer", background: plan.is_active ? "rgba(74,222,128,0.12)" : "rgba(248,113,113,0.12)", color: plan.is_active ? "#4ade80" : "#f87171", fontSize: 11, fontWeight: 700, opacity: isToggling ? 0.6 : 1 }}>
                            {isToggling
          ...
          {!loading && filtered.length > 0 && (
            <div style={{ padding: "12px 20px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ fontSize: 11, color: "rgba(244,247,247,0.2)" }}>
                Click price to edit · Click status to toggle
              </span>
            </div>
          )}
        </div>
