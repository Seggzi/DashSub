...
import {
  Users, AlertTriangle, Wifi, Activity, RefreshCw,
...
function StatCard({
  ...
  return (
    <div style={{
      ...
      border: "1px solid rgba(83,230,212,0.10)",
      borderRadius: 16,
      padding: "18px 20px",
    }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <p style={{ fontSize: 10, fontWeight: 700, color: "rgba(244,247,247,0.4)", textTransform: "uppercase", letterSpacing: ".12em" }}>
          {label}
        </p>
      ...
      {loading ? (
        ...
        <>
          <p style={{ fontSize: 26, fontWeight: 900, color, fontFamily: "'IBM Plex Mono', monospace", lineHeight: 1, marginBottom: 8 }}>
            {value}
          </p>
          {sub && (
...
function RevenueChart({ data, loading }: { data: DayRevenue[]; loading: boolean }) {
  ...
  const prev4  = data.slice(0, 3).reduce((s, d) => s + d.revenue, 0);
  const last4  = data.slice(3).reduce((s, d) => s + d.revenue, 0);
  const trend  = prev4 > 0 ? Math.round(((last4 - prev4) / prev4) * 100) : 0;
    ...
    <div style={{ background: "rgba(8,12,12,0.6)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 18, padding: 22 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        <div style={{ textAlign: "right" }}>
          <p style={{ fontSize: 18, fontWeight: 800, color: "#4ade80", fontFamily: "'IBM Plex Mono', monospace" }}>
            ₦{fmt(total)}
          </p>
          {trend !== 0 && (
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 120, paddingBottom: 28, position: "relative" }}>
          ...
          {[0, 50, 100].map(pct => (
            <div key={pct} style={{
              ...
              bottom: `calc(28px + ${pct}% * (120px - 28px) / 100)`,
          ...
          {data.map((d, i) => {
            const barH  = maxRev > 0 ? Math.max(4, Math.round(((120 - 28) * d.revenue) / maxRev)) : 4;
            const isToday = d.label === today && i === 6;
            return (
              <div key={d.date} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 0, position: "relative" }}>
                {/* Tooltip-style value on hover via title */}
                <div
                  title={`₦${fmt(d.revenue)} · ${d.count} tx`}
                  style={{
                    ...
                    background: isToday
                      ? "rgba(83,230,212,0.7)"
                      : d.revenue > 0
                        ? "rgba(83,230,212,0.35)"
                        : "rgba(255,255,255,0.06)",
                    borderRadius: "4px 4px 0 0",
                    border: isToday ? "1px solid rgba(83,230,212,0.4)" : "none",
                    cursor: "default",
                ...
                <span style={{
                  ...
                  color: isToday ? "#53E6D4" : "rgba(244,247,247,0.3)",
                  letterSpacing: ".04em",
                }}>
      ...
      {/* Mini legend */}
      <div style={{ display: "flex", gap: 16, paddingTop: 6, borderTop: "1px solid rgba(255,255,255,0.05)", marginTop: 4 }}>
        {data.slice(-3).map(d => (
          <div key={d.date} style={{ flex: 1 }}>
            <p style={{ fontSize: 10, color: "rgba(244,247,247,0.3)", marginBottom: 2 }}>{d.label}</p>
        ...
        <div style={{ flex: 1, textAlign: "right" }}>
          ...
          <p style={{ fontSize: 11, fontWeight: 700, color: "#53E6D4", fontFamily: "'IBM Plex Mono', monospace" }}>
            ₦{fmt(Math.round(total / 7))}
          </p>
        </div>
      </div>
    </div>
...
export default function AdminDashboard() {
  ...
  async function fetchStats() {
    ...
    const revenueToday = (todayTx || []).filter(t => t.status === "success").reduce((s, t) => s + t.amount, 0);
    ...
    setStats({
      ...
      successRate,
      lowBalanceCount:   lowBal?.length ?? 0,
      avgOrderValue,
      pendingCount,
    });
  }
  ...
  async function fetchLowBalance() {
    const { data, error } = await supabase
      ...
      .lt("wallet_balance", LOW_BALANCE_THRESHOLD)
  ...
  async function fetchNetworkBreakdown() {
    ...
    data.forEach(t => {
      if (!t.network) return;
      if (!map[t.network]) map[t.network] = { count: 0, revenue: 0 };
      map[t.network].count++;
      map[t.network].revenue += t.amount;
    });
  ...
  async function fetch7DayRevenue() {
    ...
    data.forEach(tx => {
      ...
      const day = days.find(d => d.date === txISO);
      if (day) {
  ...
  // ── Realtime ───────────────────────────────────────────────────────────────
  // FIX: single channel, no unsupported filters on UPDATE, covers all cases
    ...
    channelRef.current = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "transactions" },
        (payload) => {
          ...
          // Refresh stats + charts silently
          fetchStats();
          fetchNetworkBreakdown();
          fetch7DayRevenue();

          toast.success(
            `New transaction · ${tx.network} · ₦${fmt(tx.amount)}`,
            { duration: 3000 }
          );
        }
      )

      // Transaction status update (e.g. pending → success/failed)
      ...
      // Profile updated (wallet top-up, balance change, etc.)
      // FIX: removed unsupported filter — just listen to ALL updates
      ...
      .subscribe((status) => {
        ...
        if (status === "CHANNEL_ERROR") {
          ...
          toast.error("Realtime connection lost — refreshing…");
          fetchAll(true);
        }
      });
  }
  ...
  function handleRefresh() {
    setLiveCount(0); // FIX: reset live counter on manual refresh
    fetchAll();
  }
  ...
  return (
    <>
      <style>{`
        ...
        .tx-row    { transition: background .12s; animation: fadeIn .25s ease; }
      ...
      <div style={{ color: "#F4F7F7", fontFamily: "'Sora', sans-serif" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <p style={{ fontSize: 13, color: "rgba(244,247,247,0.4)" }}>
                Live overview · {lastUpdated.toLocaleTimeString()}
              </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 14 }}>
          <StatCard label="Total Users"     value={stats ? fmt(stats.totalUsers) : "—"}         sub={stats ? `+${stats.newUsersToday} registered today` : undefined} icon={Users}        color="#53E6D4" loading={loading} trend="up" />
          ...
          <StatCard label="Success Rate"    value={stats ? `${stats.successRate}%` : "—"}       sub={stats ? `${stats.pendingCount} pending` : undefined}               icon={Activity}     color={stats && stats.successRate >= 90 ? "#4ade80" : stats && stats.successRate >= 70 ? "#facc15" : "#f87171"} loading={loading} />
        </div>

        {/* Stat Grid — 2 secondary */}
        ...
        {/* 7-Day Revenue Chart — full width */}
        <div style={{ marginBottom: 20 }}>
          <RevenueChart data={weekData} loading={loading} />
        </div>

        {/* Middle Row: Network Breakdown + Low Balance */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
          ...
          <div style={{ background: "rgba(8,12,12,0.6)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 18, padding: 22 }}>
            ...
            {loading ? (
              <div style={{ padding: "30px 0", textAlign: "center" }}>
                <Loader2 style={{ width: 20, height: 20, color: "#53E6D4", margin: "0 auto", animation: "spin .8s linear infinite" }} />
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {networkData.map(n => {
                  const nc  = NETWORK_COLORS[n.name] ?? NETWORK_COLORS["9MOBILE"];
                  ...
                  return (
                    <div key={n.name}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 700, color: nc.color, fontFamily: "'IBM Plex Mono', monospace" }}>
                          ₦{fmt(n.revenue)}
                        </span>
                      </div>
          ...
          <div style={{ background: "rgba(8,12,12,0.6)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 18, padding: 22 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
              ...
              {lowBalance.length > 0 && (
                <span style={{ marginLeft: "auto", fontSize: 11, background: "rgba(248,113,113,0.1)", color: "#f87171", padding: "2px 8px", borderRadius: 6, border: "1px solid rgba(248,113,113,0.18)", fontWeight: 700 }}>
                  {lowBalance.length} users
                </span>
              )}
            </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {lowBalance.map(u => (
                  <div key={u.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: 10, background: "rgba(248,113,113,0.05)", border: "1px solid rgba(248,113,113,0.08)" }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 800, color: u.wallet_balance <= 0 ? "#f87171" : "#facc15", fontFamily: "'IBM Plex Mono', monospace" }}>
        ...
        <div style={{ background: "rgba(8,12,12,0.6)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 18, overflow: "hidden" }}>
                ...
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {["User", "Network", "Plan", "Amount", "Phone", "Status", "Time"].map(h => (
                      <th key={h} style={{ padding: "11px 16px", textAlign: "left", fontSize: 10, fontWeight: 700, color: "rgba(244,247,247,0.3)", textTransform: "uppercase", letterSpacing: ".1em", whiteSpace: "nowrap" }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {transactions.map(tx => {
                    ...
                    return (
                      <tr key={tx.id} className="tx-row" style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span style={{ padding: "3px 9px", borderRadius: 7, background: nc.bg, border: `1px solid ${nc.border}`, color: nc.color, fontSize: 11, fontWeight: 800 }}>
                            {tx.network}
                          </span>
                        </td>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span style={{ padding: "3px 10px", borderRadius: 7, background: sc.bg, color: sc.color, fontSize: 11, fontWeight: 700 }}>
                            {sc.label}
                          </span>
                        </td>
          ...
          <div style={{ padding: "12px 22px", borderTop: "1px solid rgba(255,255,255,0.04)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 12, color: "rgba(244,247,247,0.25)" }}>
              Last {transactions.length} · auto-updates via realtime
            </span>
