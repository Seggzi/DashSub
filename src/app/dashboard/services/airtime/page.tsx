    ...
    return (
      <div className="min-h-screen bg-brand-primary flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-brand-gray/60">Loading...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-brand-primary text-white">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-0 w-96 h-96 bg-brand-mint/5 rounded-full blur-3xl"></div>
      ...
      <div className="relative z-10 max-w-2xl mx-auto p-6 pb-20">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-3 mb-2">
            <div className="p-3 bg-gradient-to-br from-brand-mint to-emerald-400 rounded-2xl shadow-lg shadow-brand-mint/20">
              <Phone className="w-6 h-6 text-brand-carbon" />
            </div>
            <div>
              <h1 className="text-3xl font-bold">
                Buy Airtime
              </h1>
              <p className="text-brand-gray/60 text-sm mt-1">Instant top-up • All networks</p>
            </div>
          </div>
        </div>
        ...
        {/* Purchase Form */}
        <div className="bg-brand-carbon backdrop-blur-xl rounded-3xl p-6 border border-white/5 mb-8">
          {/* Phone Number Input */}
          <div className="mb-6">
            <label className="block text-sm font-semibold text-brand-gray/60 mb-3 flex items-center gap-2">
            ...
            <div className="relative">
              <input
                ...
                maxLength={11}
                className="w-full bg-brand-primary border-2 border-white/5 rounded-2xl px-5 py-4 text-white text-lg placeholder:text-brand-gray/50 focus:outline-none focus:border-brand-mint transition-all duration-300"
              />
              {selectedNetwork && (
                ...
                <button
                  ...
                  className={`relative p-4 rounded-2xl border-2 transition-all duration-300 group ${
                    selectedNetwork === network.id
                      ...
                      : 'border-white/5 bg-brand-primary hover:border-white/10 hover:scale-105'
                  }`}
                >
          ...
          {/* Amount Input */}
          <div className="mb-6">
            <label className="block text-sm font-semibold text-brand-gray/60 mb-3">
              Amount
            </label>
            <div className="relative">
              <span className="absolute left-5 top-1/2 -translate-y-1/2 text-2xl font-bold text-brand-gray/50">
                ₦
              </span>
              <input
                type="number"
                ...
                placeholder="0"
                className="w-full bg-brand-primary border-2 border-white/5 rounded-2xl pl-12 pr-5 py-4 text-white text-2xl font-bold placeholder:text-brand-gray/50 focus:outline-none focus:border-brand-mint transition-all duration-300"
                min="50"
              />
            </div>
            <p className="text-xs text-brand-gray/60 mt-2 flex items-center gap-1">
          ...
          <div className="mb-8">
            <p className="text-xs font-semibold text-brand-gray/60 mb-3">Quick Select</p>
            <div className="grid grid-cols-4 gap-2">
              {[100, 200, 500, 1000].map((quickAmount) => (
                <button
                  key={quickAmount}
                  onClick={() => setAmount(quickAmount.toString())}
                  className="py-3 px-4 bg-brand-primary hover:bg-white/5 rounded-xl text-sm font-bold transition-all duration-300 hover:scale-105 border border-white/5"
                >
                  ₦{quickAmount}
                </button>
              ))}
          ...
          {/* Price Breakdown */}
          {numAmount >= 50 && (
            <div className="mb-6 p-5 bg-brand-primary rounded-2xl border border-brand-mint/20">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-brand-gray/60 text-sm">Airtime</span>
                  <span className="font-bold text-white text-lg">₦{numAmount.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-brand-gray/60 text-sm flex items-center gap-1">
                    Service Fee (2%)
                  </span>
        ...
        <div>
          <div className="flex items-center gap-3 mb-5">
            <div className="p-2 bg-brand-carbon rounded-xl border border-white/5">
          ...
          {transactions.length === 0 ? (
            <div className="bg-brand-carbon rounded-2xl p-12 text-center border border-white/5">
              <div className="w-20 h-20 bg-brand-primary rounded-full flex items-center justify-center mx-auto mb-4">
                <Phone className="w-10 h-10 text-brand-gray/60" />
              </div>
              <p className="text-brand-gray/60 font-medium">No purchase history yet</p>
            ...
            <div className="space-y-3">
              {transactions.map((tx) => {
                ...
                return (
                  <div 
                    key={tx.id} 
                    className="bg-brand-carbon rounded-2xl p-5 border border-white/5 hover:border-white/10 transition-all duration-300 hover:scale-[1.01] group"
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-brand-gray/60 capitalize mt-1">{tx.network}</p>
                        </div>
                      </div>
                      <span
                    ...
                    <div className="flex items-center gap-2 text-xs text-brand-gray/60">
