...
export default function Navbar() {
  ...
  // 1. Handle Hydration (Prevents Edge/Chrome mismatch on load)
  useEffect(() => {
    setMounted(true);
  ...
  // 2. Don't render auth buttons until mounted to avoid showing wrong state
  ...
  return (
    <header className="fixed w-full top-0 z-[100] p-4 md:p-6">
      ...
      >
        ...
        {/* Mobile Dropdown */}
        <div className={`lg:hidden absolute top-[115%] left-0 right-0 transition-all duration-500 origin-top ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-5 pointer-events-none'}`}>
            ...
            <div className="flex flex-col gap-5">
              ...
              {!session ? (
                <div className="flex flex-col gap-4">
                  <Link href="/auth" onClick={() => setIsOpen(false)} className="text-center text-white/70 py-2 font-bold">
                    Login
                  </Link>
                  <Link href="/auth" onClick={() => setIsOpen(false)} className="w-full bg-brand-mint text-brand-carbon py-5 rounded-2xl font-black text-center text-lg">
                    Join Us Now
                  </Link>
                </div>
              ) : (
                <Link href="/dashboard" onClick={() => setIsOpen(false)} className="w-full bg-brand-mint text-brand-carbon py-5 rounded-2xl font-black text-center text-lg">
                  Go to Dashboard
                </Link>
              )}
            </div>
